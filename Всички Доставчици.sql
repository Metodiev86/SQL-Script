...
st.Qtty AS 'Количество на Склад', g.PriceIn AS 'Доставна цена', gGroup.Code,
...
INNER JOIN GoodsGroups AS gGroup ON g.GroupID = gGroup.ID
INNER JOIN Store AS st ON g.ID = st.GoodID AND obj.ID = st.ObjectID
WHERE  operTYPE.BG IN ('Продажба', 'Заявка за доставка', 'Поръчка') and g.Deleted = 0 
 
GROUP BY  oper.Qtty, oper.Date, g.ID, g.Code, g.Name, g.Measure1, 
st.Qtty, g.PriceIn, gGroup.Name, obj.Name, obj.ID,
par.Company, operTYPE.BG, g.Ratio, gGroup.Code